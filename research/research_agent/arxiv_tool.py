...
class ArxivTool(Tool):
    ...
    def __init__(self, model=None):
        super().__init__()
        self.model = model  # Store the model adapter (though not used in current implementation)
    inputs = {
        "query": {
            "type": "string",
    ...
    def forward(self, query: str, max_results: int = 5, debug: bool = True) -> str:
        ...
        time_keywords = ["latest", "recent", "current", "new", "state-of-the-art", "cutting-edge", "emerging"]
        quality_keywords = ["best", "most important", "seminal", "influential", "foundational", "top", "highly cited", "landmark", "groundbreaking", "classic"]
